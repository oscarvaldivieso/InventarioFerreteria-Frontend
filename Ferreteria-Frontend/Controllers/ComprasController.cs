...
namespace Ferreteria_Frontend.Controllers
{
    public class ComprasController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(CompraViewModel comp, List<CompraDetalleViewModel> detalles)
        {
            ViewBag.proveedores = await ObtenerProveedoresAsync();
            ViewBag.productos = await ObtenerProductosAsync();
            ...
            if (ModelState.IsValid)
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        detalle.Usua_Creacion = 1;
                        detalle.Feca_Creacion = DateTime.Now;
                        detalle.Comp_Id = comp.Comp_Id; // Asignar el ID de la compra al detalle

                        var jsonDetalle = JsonConvert.SerializeObject(detalle);
                        var contentDetalle = new StringContent(jsonDetalle, Encoding.UTF8, "application/json");
                        await _httpClient.PostAsync("InsertarCompraDetalle", contentDetalle);
                    }
